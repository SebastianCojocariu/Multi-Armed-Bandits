...
class BootstrappedUCB(ContextualBandit):
	def __init__(self, policy_args, model_args):
		super().__init__(policy_args=policy_args, model_args=model_args)
		self.percentile = policy_args["percentile"]
		self.m = policy_args["m"]
		self.policy_name = "BootstrappedUCB" + "(m: {}, percentile: {}, p: {}, oracle: {})".format(self.m, self.percentile, self.p, model_args["class_model_name"])


	def compute_score(self, arm, context, normalising_factor):
		scores = np.asarray([model.predict(context) for model in self.oracles[arm]["models"]])
		score = np.percentile(scores, self.percentile)
